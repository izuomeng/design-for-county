...
| 字段 | key | 题面（title） | 控件 | 默认/草稿 |
|---|---|---|---|---|
| 产品名称 | `productName` | 产品名称 | **文本输入**（手填） | 按品类+产地拟建议名（如「宜君玉米糁」） |
| 净含量 | `spec` | 净含量 | **文本输入**（手填，**不要 enum**） | 你按品类拟一个数字草稿（如 `250g` / `500g`），用户改 |
| 包装形态 | `packForm` | 包装形态 | **单选 enum**（下方 7 个全列上） | 袋装 |

- **净含量 `spec`**：题面就叫「净含量」，是**纯手填的数字+单位**（如 `250g`、`500g`、`1kg`、`500ml`）。askUser 里这一项**只用 `type:"string"` 文本框，绝不要给 enum**；你先填一个草稿值放 `default`，用户直接改。
- **包装形态 `packForm`**：单选，enum **必须完整给出这 7 个**（不要少给）：
  `["袋装","普通盒装","礼盒装","瓶装","罐装","天然材质袋","箱装"]`
- 这 3 个尽量**一次 `askUser`** 收齐；用户开场已说明其中某项就直接带入、跳过。
...
包装上需清晰印刷中文：主标题"{productName}"、净含量"{spec}"，中文字体清晰、排版整齐、无错字、无多余文字。
```
